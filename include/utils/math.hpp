...
public:
    ...
    // エイリアシング防止用キースケーリングテーブル
    // 高いノートほど変調深度を減らしてエイリアシングを防ぐ
    // 値は 0-1024 (1024 = 100%)
    // ノート60(C4)以下: 100%, ノート84(C6)以上: 25%
    static inline const int16_t MOD_KEY_SCALE_TABLE[128] = {
        // 0-11 (C-1 to B-1): 100%
        ...
        // 48-59 (C3 to B3): 100%
        1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024,
        // 60-71 (C4 to B4): 100% -> 85% (徐々に減少)
        1024, 1000, 976, 952, 928, 904, 880, 856, 832, 808, 784, 760,
        // 72-83 (C5 to B5): 72% -> 40%
        736, 704, 672, 640, 608, 576, 544, 512, 480, 448, 416, 384,
        // 84-95 (C6 to B6): 35% -> 25%
        358, 345, 332, 320, 307, 294, 282, 269, 269, 269, 269, 269,
        // 96-107 (C7 to B7): 25% (固定)
