...
public:
    ...
    // 低レベル域 (0-19) の非線形変換用テーブル
    // 低い値ほど変化が大きい（圧縮されている）
    // 高レベル域 (20-99) は線形なので計算で求める
    // オシレーター出力レベル、エンベロープレベル両方で使用
    static constexpr uint8_t LOW_LEVEL_LUT[20] = {
    ...
    static inline Gain_t levelToLinear(uint8_t level) {
        ...
        // TL (Total Level): 減衰量。0=最大音量, 127=無音
        uint8_t tl;
        if (level < 20) {
            // 低レベル域: 非線形テーブル参照
            tl = 127 - LOW_LEVEL_LUT[level];
        } else {
            // 高レベル域: 線形 (level=20→tl=79, level=99→tl=0)
            tl = 99 - level;
        }
