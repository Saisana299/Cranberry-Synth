    ...
    void redrawContent(GFXcanvas16& canvas) {
        ...
        for (int8_t i = 0; i < 5; i++) {
            ...
            if (i == 0) {
                ...
                sprintf(value, "%.0fHz", synth.getHpfCutoff());
            } else if (i == 2) {
                sprintf(value, "%.2f", synth.getHpfResonance());
            } else if (i == 3) {
                sprintf(value, "%d%%", (int)(synth.getHpfMix() * 100 / 1024));
            }
...
public:
    ...
    void handleInput(uint8_t btn) override {
        ...
        // 値を減らす
            ...
            if (cursor == 1) {
                synth.getFilter().setHighPass(
                    constrain(synth.getHpfCutoff() - step * 100, 20.0f, 20000.0f),
                    synth.getHpfResonance()
                );
            }
            else if (cursor == 2) {
                synth.getFilter().setHighPass(
                    synth.getHpfCutoff(),
                    constrain(synth.getHpfResonance() - 0.1f * step, 0.1f, 10.0f)
                );
            }
            else if (cursor == 3) {
                synth.getFilter().setHpfMix(
                    constrain(synth.getHpfMix() - step * 10, (int16_t)0, (int16_t)1024)
                );
            }
