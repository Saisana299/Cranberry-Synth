...
private:
    ...
    void redrawContent(GFXcanvas16& canvas) {
        ...
        const char* labels[] = {"Enable", "Cutoff", "Resonance", "Mix", "< Back"};
        ...
        for (int8_t i = 0; i < 5; i++) {
            ...
            if (i == 0) {
                ...
                sprintf(value, "%.0fHz", synth.getLpfCutoff());
            } else if (i == 2) {
                sprintf(value, "%.2f", synth.getLpfResonance());
            } else if (i == 3) {
                sprintf(value, "%d%%", (int)(synth.getLpfMix() * 100 / 1024));
            }
...
public:
    ...
    void handleInput(uint8_t btn) override {
        ...
        // 値を減らす
            ...
            if (cursor == 1) {
                synth.getFilter().setLowPass(
                    constrain(synth.getLpfCutoff() - step * 100, 20.0f, 20000.0f),
                    synth.getLpfResonance()
                );
            }
            else if (cursor == 2) {
                synth.getFilter().setLowPass(
                    synth.getLpfCutoff(),
                    constrain(synth.getLpfResonance() - 0.1f * step, 0.1f, 10.0f)
                );
            }
            else if (cursor == 3) {
                synth.getFilter().setLpfMix(
                    constrain(synth.getLpfMix() - step * 10, (int16_t)0, (int16_t)1024)
                );
            }
