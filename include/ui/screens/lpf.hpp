...
public:
    ...
    void handleInput(uint8_t button) override {
        ...
        // 左右ボタン：値の変更
            ...
            if (cursor == C_ENABLED) {
                synth.setLpfEnabled(!synth.isLpfEnabled());
                changed = true;
            }
            else if (cursor == C_CUTOFF) {
                float cutoff = synth.getLpfCutoff();
                ...
                synth.getFilter().setLowPass(cutoff, synth.getLpfResonance());
                changed = true;
            }
            else if (cursor == C_RESONANCE) {
                float resonance = synth.getLpfResonance() - RESONANCE_STEP;
                if (resonance < Filter::RESONANCE_MIN) resonance = Filter::RESONANCE_MIN;
                synth.getFilter().setLowPass(synth.getLpfCutoff(), resonance);
                changed = true;
            }
...
private:
    ...
    void drawAllItems(GFXcanvas16& canvas) {
        Synth& synth = Synth::getInstance();
        drawToggleItem(canvas, "ENABLED", synth.isLpfEnabled(), 0, cursor == C_ENABLED);
        drawFreqItem(canvas, "CUTOFF", synth.getLpfCutoff(), 1, cursor == C_CUTOFF);
        drawFloatItem(canvas, "Q", synth.getLpfResonance(), 2, cursor == C_RESONANCE);
        drawPercentItem(canvas, "MIX", synth.getLpfMix(), 3, cursor == C_MIX);
    }
    ...
    void updateCursorElement(GFXcanvas16& canvas, int8_t cursorPos) {
        ...
        if (cursorPos == C_ENABLED) {
            drawToggleItem(canvas, "ENABLED", synth.isLpfEnabled(), 0, isSelected);
        }
        else if (cursorPos == C_CUTOFF) {
            drawFreqItem(canvas, "CUTOFF", synth.getLpfCutoff(), 1, isSelected);
        }
        else if (cursorPos == C_RESONANCE) {
            drawFloatItem(canvas, "Q", synth.getLpfResonance(), 2, isSelected);
        }
        else if (cursorPos == C_MIX) {
            drawPercentItem(canvas, "MIX", synth.getLpfMix(), 3, isSelected);
        }
