...
class ChorusScreen : public Screen {
private:
    // 定数
...
public:
    ...
    void handleInput(uint8_t button) override {
        ...
        // 左ボタン：値を減少
            ...
            if (cursor == C_ENABLED) {
                synth.setChorusEnabled(!synth.isChorusEnabled());
                changed = true;
            }
            else if (cursor == C_RATE) {
                int val = synth.getChorusRate() - RATE_STEP;
                if (val < CHORUS_RATE_MIN) val = CHORUS_RATE_MIN;
                synth.getChorus().setRate(static_cast<uint8_t>(val));
                changed = true;
            }
            else if (cursor == C_DEPTH) {
                int val = synth.getChorusDepth() - DEPTH_STEP;
                if (val < CHORUS_DEPTH_MIN) val = CHORUS_DEPTH_MIN;
                synth.getChorus().setDepth(static_cast<uint8_t>(val));
                changed = true;
            }
            else if (cursor == C_MIX) {
                int32_t val = static_cast<int32_t>(synth.getChorusMix()) - MIX_STEP;
                if (val < 0) val = 0;
                synth.getChorus().setMix(static_cast<Gain_t>(val));
                changed = true;
            }
        }
        ...
        // 右ボタン：値を増加
            ...
            if (cursor == C_ENABLED) {
                ...
                int val = synth.getChorusRate() + RATE_STEP;
                if (val > CHORUS_RATE_MAX) val = CHORUS_RATE_MAX;
                synth.getChorus().setRate(static_cast<uint8_t>(val));
                changed = true;
            }
            else if (cursor == C_DEPTH) {
                int val = synth.getChorusDepth() + DEPTH_STEP;
                if (val > CHORUS_DEPTH_MAX) val = CHORUS_DEPTH_MAX;
                synth.getChorus().setDepth(static_cast<uint8_t>(val));
                changed = true;
            }
            else if (cursor == C_MIX) {
...
private:
    ...
    void drawAllItems(GFXcanvas16& canvas) {
        Synth& synth = Synth::getInstance();
        drawToggleItem(canvas, "ENABLED", synth.isChorusEnabled(), 0, cursor == C_ENABLED);
        drawParamItem(canvas, "RATE", synth.getChorusRate(), "", 1, cursor == C_RATE);
        drawParamItem(canvas, "DEPTH", synth.getChorusDepth(), "", 2, cursor == C_DEPTH);
        drawParamItem(canvas, "MIX", (synth.getChorusMix() * 100) / Q15_MAX, "%", 3, cursor == C_MIX);
    }
    ...
    void updateCursorElement(GFXcanvas16& canvas, int8_t cursorPos) {
        ...
        if (cursorPos == C_ENABLED) {
            drawToggleItem(canvas, "ENABLED", synth.isChorusEnabled(), 0, isSelected);
        }
        else if (cursorPos == C_RATE) {
            drawParamItem(canvas, "RATE", synth.getChorusRate(), "", 1, isSelected);
        }
        else if (cursorPos == C_DEPTH) {
            drawParamItem(canvas, "DEPTH", synth.getChorusDepth(), "", 2, isSelected);
        }
        else if (cursorPos == C_MIX) {
            drawParamItem(canvas, "MIX", (synth.getChorusMix() * 100) / Q15_MAX, "%", 3, isSelected);
        }
